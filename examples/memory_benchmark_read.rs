...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Memory after open: {:.2} MB",
        mem_after_open as f64 / 1024.0
    );
    println!(
        "   Memory delta: {:.2} MB (SST + metadata loaded)",
    ...
    println!(
        "   Memory before streaming: {:.2} MB",
        mem_before_read as f64 / 1024.0
    );
    ...
    for row_result in reader.stream_rows("Sheet1")? {
        ...
        if row_count % 100_000 == 0 {
            if let Some(mem) = get_memory_kb() {
                ...
                println!(
                    "   Progress: {} rows ({:.1}M cells) - Memory: {:.2} MB",
                    row_count,
    ...
    println!(
        "   Speed: {:.0} rows/sec",
        row_count as f64 / read_time.as_secs_f64()
    );
    println!(
    ...
    println!(
        "   Memory peak during read: {:.2} MB",
        peak_mem as f64 / 1024.0
    );
    println!(
        "   Memory after read: {:.2} MB",
        mem_after_read as f64 / 1024.0
    );
    println!(
        "   Memory delta (open): {:.2} MB",
        (mem_after_open - mem_start) as f64 / 1024.0
    );
    println!(
        "   Memory delta (peak): {:.2} MB",
    ...
    println!(
        "✅ Peak memory: {:.2} MB (constant!)",
        peak_mem as f64 / 1024.0
    );
    println!(
        "✅ Read speed: {:.0} rows/sec",
        row_count as f64 / read_time.as_secs_f64()
    );
    println!(
        "✅ Memory efficiency: {:.1}x smaller than file size",
        file_size as f64 / ((peak_mem - mem_start) as f64 * 1024.0)
    );
    println!();
    println!("🎯 Key Achievement:");
    println!(
        "   {:.2} MB file → {:.2} MB RAM = {:.0}% memory reduction!",
        file_size as f64 / (1024.0 * 1024.0),
        peak_mem as f64 / 1024.0,
